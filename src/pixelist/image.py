...
NumpyArray = Annotated[
    ndarray,
    PlainValidator(lambda x: x if isinstance(x, ndarray) else 
...
class ImageSuperposition(BaseModel):
    ...
    batches: Annotated[
        List[ImageBatch],
        PlainValidator(lambda x: x if isinstance(x, list) and len(x) > 0 and 
                      all(isinstance(b, ImageBatch) for b in x) 
